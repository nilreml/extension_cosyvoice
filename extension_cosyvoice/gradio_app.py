...
def get_cosyvoice_model(model_name="CosyVoice2-0.5B", load_jit=False, load_trt=False, fp16=False):
    ...
    if cosyvoice_model is None:
        ...
        cosyvoice_model = CosyVoice2(model_path, load_jit=load_jit, load_trt=load_trt, fp16=fp16)
...
def load_prompt_audio(prompt_audio):
    ...
    return load_wav(prompt_audio, 16000) if isinstance(prompt_audio, str) else prompt_audio
...
def process_results(generation_results, sample_rate):
    ...
    if len(generation_results) > 1:
        all_audio = torch.cat([result['tts_speech'] for result in generation_results], dim=1)
        return (sample_rate, all_audio[0].cpu().numpy())
    else:
        return (sample_rate, generation_results[0]['tts_speech'][0].cpu().numpy())

def inference_zero_shot(prompt_audio, prompt_text, gen_text, stream=False, model_name="CosyVoice2-0.5B"):
    model = get_cosyvoice_model(model_name)
    prompt_speech = load_prompt_audio(prompt_audio)
    generation_results = list(model.inference_zero_shot(gen_text, prompt_text, prompt_speech, stream=stream))
    return process_results(generation_results, model.sample_rate)

def inference_instruct(prompt_audio, prompt_text, gen_text, instruct_text, stream=False, model_name="CosyVoice2-0.5B"):
    model = get_cosyvoice_model(model_name)
    prompt_speech = load_prompt_audio(prompt_audio)
    generation_results = list(model.inference_instruct2(gen_text, instruct_text, prompt_speech, stream=stream))
    return process_results(generation_results, model.sample_rate)

def inference_cross_lingual(prompt_audio, prompt_text, gen_text, stream=False, model_name="CosyVoice2-0.5B"):
    model = get_cosyvoice_model(model_name)
    prompt_speech = load_prompt_audio(prompt_audio)
    generation_results = list(model.inference_cross_lingual(gen_text, prompt_speech, stream=stream))
    return process_results(generation_results, model.sample_rate)

def inference_text_generator(prompt_audio, prompt_text, gen_text, stream=False, model_name="CosyVoice2-0.5B", chunk_size=100):
    model = get_cosyvoice_model(model_name)
    prompt_speech = load_prompt_audio(prompt_audio)
    ...
    generation_results = list(model.inference_zero_shot(text_generator(), prompt_text, prompt_speech, stream=stream))
    return process_results(generation_results, model.sample_rate)
...
def ui_app_tts():
    with gr.Blocks() as app_tts:
        ...
        ref_text_input = gr.Textbox(label="Reference Text", lines=2, placeholder="Text that corresponds to the reference audio")
        gen_text_input = gr.Textbox(label="Text to Generate", lines=10)
        
        with gr.Accordion("Advanced Settings", open=False):
            model_choice = gr.Radio(
                choices=["CosyVoice2-0.5B", "CosyVoice-300M", "CosyVoice-300M-25Hz", "CosyVoice-300M-SFT", "CosyVoice-300M-Instruct", "CosyVoice-ttsfrd"], 
        ...
        generate_btn.click(
            inference_zero_shot,
            inputs=[ref_audio_input, ref_text_input, gen_text_input, stream_checkbox, model_choice],
...
def ui_app_instruct():
    with gr.Blocks() as app_instruct:
        ...
        generate_btn.click(
            inference_instruct,
            inputs=[ref_audio_input, ref_text_input, gen_text_input, instruct_text_input, stream_checkbox, model_choice],
            outputs=audio_output,
        )
...
def ui_app_cross_lingual():
    with gr.Blocks() as app_cross:
        ...
        gr.Markdown("Use special tags like [laughter] to control fine-grained speech characteristics")
        ref_audio_input = gr.Audio(label="Reference Audio", type="filepath")
        ref_text_input = gr.Textbox(label="Reference Text", lines=2, placeholder="Text that corresponds to the reference audio")
...
def ui_app_chunk():
    with gr.Blocks() as app_chunk:
        ...
        def inference_chunked(prompt_audio, prompt_text, gen_text, stream, model_name, chunk_size):
            return inference_text_generator(prompt_audio, prompt_text, gen_text, stream, model_name, int(chunk_size))

        generate_btn.click(
            inference_chunked,
            inputs=[ref_audio_input, ref_text_input, gen_text_input, stream_checkbox, model_choice, chunk_size],
            outputs=audio_output,
        )
